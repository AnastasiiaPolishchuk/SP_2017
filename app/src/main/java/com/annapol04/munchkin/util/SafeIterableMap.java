...
public class SafeIterableMap<K, V> implements Iterable<Map.Entry<K, V>> {
    ...
    protected SafeIterableMap.Entry<K, V> get(K k) {
        SafeIterableMap.Entry<K, V> currentNode = mStart;
    ...
    protected SafeIterableMap.Entry<K, V> put(@NonNull K key, @NonNull V v) {
        SafeIterableMap.Entry<K, V> newEntry = new SafeIterableMap.Entry<>(key, v);
        mSize++;
        if (mEnd == null) {
            mStart = newEntry;
            mEnd = mStart;
    ...
    public V remove(@NonNull K key) {
        SafeIterableMap.Entry<K, V> toRemove = get(key);
        if (toRemove == null) {
            return null;
        }
        mSize--;
        if (!mIterators.isEmpty()) {
            for (SafeIterableMap.SupportRemove<K, V> iter : mIterators.keySet()) {
    ...
    public Iterator<Map.Entry<K, V>> iterator() {
        SafeIterableMap.ListIterator<K, V> iterator = new SafeIterableMap.AscendingIterator<>(mStart, mEnd);
    ...
    public Iterator<Map.Entry<K, V>> descendingIterator() {
        SafeIterableMap.DescendingIterator<K, V> iterator = new SafeIterableMap.DescendingIterator<>(mEnd, mStart);
        mIterators.put(iterator, false);
        return iterator;
    }
    ...
    public SafeIterableMap.IteratorWithAdditions iteratorWithAdditions() {
        ...
        SafeIterableMap.IteratorWithAdditions iterator = new SafeIterableMap.IteratorWithAdditions();
        mIterators.put(iterator, false);
        return iterator;
    }
    ...
    private abstract static class ListIterator<K, V> implements Iterator<Map.Entry<K, V>>,
        ...
        abstract SafeIterableMap.Entry<K, V> forward(SafeIterableMap.Entry<K, V> entry);

        abstract SafeIterableMap.Entry<K, V> backward(SafeIterableMap.Entry<K, V> entry);
    }
    ...
    private class IteratorWithAdditions implements Iterator<Map.Entry<K, V>>, SafeIterableMap.SupportRemove<K, V> {
    ...
    static class Entry<K, V> implements Map.Entry<K, V> {
        ...
        public boolean equals(Object obj) {
            ...
            SafeIterableMap.Entry entry = (SafeIterableMap.Entry) obj;
